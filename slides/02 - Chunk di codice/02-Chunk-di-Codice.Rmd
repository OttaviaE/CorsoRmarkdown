...
- Create un nuovo documento di testo `RMmarkdown`
- Scegliete un titolo e sottotitolo inerente il vostro dataset
...
- Se ci sono degli errori, non ve ne accrogete
...
stargazer(mtcars, type="latex", summary = TRUE, 
           title= "Tabella di summary", digits = 2, header=TRUE)
```
````
...
```{r, results='asis', echo = TRUE, class.source='myClass'}
m1 = lm(mpg ~ cyl, data = mtcars)
stargazer(m1, type="html", summary = TRUE, 
           title= "Risultati del modello", digits = 2, header=TRUE)
```

## `stargazer` model comparison

```{r, echo = TRUE, results='asis', echo = TRUE, class.source='myClass'}
m0 = lm(mpg ~ 1, data = mtcars)
m1 = lm(mpg ~ cyl, data = mtcars)

stargazer(m0, m1, 
          type = "html",
          title="Model comparison", digits=2, intercept.top = TRUE, 
          intercept.bottom = FALSE, header = TRUE)
```
...
- Tabella di model coparison con modello nullo `m0` (`m0 = lm(y ~ 1, data = dati)`)
...
Se in un chunk precedente avessimo assegnato `x = mean(mtcars$mpg)`, avrremmo potuto semmplicemente scrivere `` `r "\u0060r mean(x)\u0060"` ``
